...
from utils.error_helper import PathError
from utils.logger_helper import clog
from aiohttp import FormData


async def get(url: str, params: dict) -> ClientResponse:
    """
    GET 方式获取URL地址内容
    ...
    async with session.get(url, params=params) as resp:
        clog.info("GET {0}".format(resp.url))
        return resp


async def post(url: str, params: dict=None, data: str=None) -> ClientResponse:
    """
    POST 方式获取URL地址内容
    :param url: http(s)://url
    :param params:  params
    ...
    async with session.post(url, data=data, params=params) as resp:
        clog.info("POST {0}".format(resp.url))
        return resp
...
async def post_file(url: str, file_path: str, params: dict=None) -> ClientResponse:
    ...
    if not os.path.exists(file_path):
        clog.error("image_path: {0} 上传路径不存在".format(file_path))
        raise PathError("image_path: {0} 上传路径不存在".format(file_path))
    image_name = file_path.split("/")[-1]
    clog.info("image_name: {0}".format(image_name))
    # 获取 aiohttp client session 对象
    session = Session.get_session()
    with open(file_path, "rb") as f:
        clog.info("{0} 文件准备上传".format(file_path))
        data = FormData()
